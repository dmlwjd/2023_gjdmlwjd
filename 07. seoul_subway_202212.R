...
table(subway$Line)
subway$Line <-
  ifelse(subway$Line =="9호선2~3단계","9호선",subway$Line)

#파생변수3. station변수
table(subway$Station)

#파생변수4. 총승하차승객수
subway$total_passenger <- 
  subway$on_board+subway$getting_off

#분석데이터 확인
str(subway)

#데이터 분석
#1.지하철역의 하루 평균 승차/하차승객 수 
subway %>%
  summarise(on_m=mean(on_board), off_m=mean(getting_off))

#2. 승차승객수가 가장 많았던 역의 노선
max(subway$on_board)
subway %>% 
...
#역별 일평군 전체 승객수 상위 10개 역을 막대그래프로 작성
